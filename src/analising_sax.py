'''
Permite profundizar en el análisis de la codificación SAX
Saca las gráficas de los resultados obtenidos y un a
'''
import pandas as pd
...
if __name__ == '__main__':
    ...
    for trozo in 'C R G'.split():
        ...
        print('='*20,
        f'Analizando trozo {trozo}')
        df = read_data(f'./experiment_results/SAX/mutual_information_{trozo}.csv',
        ...
        print(f'La IM máxima del trozo {trozo} es {maximo}.') 
        sorted_df = df.sort_values(by=IM_column,  ascending=False)
        print(f'El top de IM es de {top}')
        print(sorted_df.head(10)) 
        # Mínimo 
        minimo = df.iloc[df[IM_column].idxmin()] 
        print(f'La IM máxima del trozo {trozo} es {minimo}.') 
        sorted_df = df.sort_values(by=IM_column,  ascending=True)
        print(f'El top de IM es de {top}')
        print(sorted_df.head(10)) 
        ...
        print(f'Imágenes del trozo {trozo} guardadas en {path} con éxito')